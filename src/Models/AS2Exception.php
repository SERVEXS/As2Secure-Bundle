...
class AS2Exception extends \Exception
{
    ...
    protected static array $level_failure = [
        101 => 'unsupported format',          // sha1, md5
        102 => 'unsupported MIC-algorithms',
    ];

    protected static array $level_warning = [
    ...
    {
        if (array_key_exists($this->code, self::$level_error)) {
            return self::STATUS_ERROR;
        }
        if (array_key_exists($this->code, self::$level_failure)) {
            return self::STATUS_FAILURE;
        }
        if (array_key_exists($this->code, self::$level_warning)) {
    ...
    {
        if (array_key_exists($this->code, self::$level_error)) {
            return self::$level_error[$this->code];
        }
        if (array_key_exists($this->code, self::$level_failure)) {
            return self::$level_failure[$this->code];
        }
        if (array_key_exists($this->code, self::$level_warning)) {
            return self::$level_warning[$this->code];
        }

        return self::$level_error[self::DEFAULT_ERROR];
    }
}
